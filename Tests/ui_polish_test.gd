...
func _run() -> void:
	...
	_check(
		...
		"Prototype buttons and panels use Godot-generated flat styles"
	)
	...
	for button in menu_buttons:
		...
		_check(
			button.has_node("BiomechShadow"),
			"Main menu button has a drop shadow: %s" % button.name
		)
		_check(
			button.size.x <= 420.5
	...
	_check(
		menu.menu_shell.get_global_rect().grow(-34.0).encloses(
			menu.title_label.get_global_rect()
		),
		"Fleshdrive title stays inside the main-menu frame safe area"
	)
	...
	_check(
		dialogue.body_label.get_theme_color("default_color").get_luminance()
		> 0.7,
		"Dialogue copy remains readable on the generated dark panel"
	)
	dialogue.queue_free()
	await process_frame
	_finish()
